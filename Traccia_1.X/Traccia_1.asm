...
; in modalità sleep per essere risvegliato dal successivo
; interrupt. Ma non tutte le periferiche funzionano durante lo
; sleep, come il TIMER0 (usato per il debouncing), che dipende
; dal clock della CPU.
; Utilizziamo perciò un bit che indica quando il programma puo'
...
; Formattazione del contatore per la stampa
    ...
    formatta_contatore:	
		    ...
		    movwf contatoreU		    ; Carico il valore di counter nel contatore delle unità
		...
		loop_div_10:			    ; Inizio della routine di divisione per 10 tramite sottrazione
		    ...
		    movwf contatoreU		    ; Altrimenti, se C = 1 (non c'è stato prestito) contatoreU = contatoreU - 10
...
;   esegue solo un loop vuoto).
	...
	swapf	STATUS,w			    ; Inverte i nibble di STATUS salvando il risultato in w.
						    ; In questo modo, si può copiare STATUS senza alterarlo
...
; N.B. -> Se il bit 0 della porta B è = 0 (pulsante premuto)
